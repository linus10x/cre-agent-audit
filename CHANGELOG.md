...
- **`./demo.sh` — the public three-scene demo.** `examples/governance_demo.py` (lives in `examples/`, not `src/`, matching this repo's convention of keeping runnable demonstration scripts out of the distributed wheel), tested in `tests/test_demo.py`. Real public API only, no test-only seams: (1) an agent submits a decision under an action_class no `ConstraintCheck` was ever registered for — the Sovereign Veto fails closed (`UNKNOWN-ACTION-CLASS`); (2) an attacker deletes the JSONL line holding a real veto/revocation entry, placed mid-chain (not last — a tail deletion breaks nothing downstream) — a fresh `verify_chain()` catches the broken hash link; (3) an attacker regenerates the *entire* chain via the real public `AuditLedger.append(now=...)` parameter, producing a chain that is internally self-consistent (`verify_chain()` alone is fooled — ADR-0003's own documented limit), and forges a `witness_anchor` ledger entry claiming a chain head was anchored — caught because that claim is checked against the *independent* witness register's own held records (not recomputed from the ledger a second time), and the attacker never had a path to submit anything to it. RFC 3161 timestamping is exercised against a genuine loopback demo TSA (stdlib `http.server`, `127.0.0.1` only, `server_close()`'d on exit — zero external network, zero extra installs, zero leaked sockets) using the repo's own `rfc3161_codec.py`. Each run uses `tempfile.mkdtemp()` so concurrent runs never collide.
