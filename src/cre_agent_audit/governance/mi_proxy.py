"""MI Proxy — Module Integrity verifier chain-of-custody (ADR-0013).
...
stdlib-only default backend (SHA-256 + HMAC). Opt-in backends delegate
to SLSA / in-toto / Sigstore cosign via the ``[attestation]`` extra.

Hooked into ``AuditLedger.verify_chain(mi_proxy=...)``: when supplied,
attestation failure raises ``IntegrityVerificationError`` and refuses to
return a verified result. Fail-closed.

> Patterns are software, not legal advice. Regulatory citations live in
> ADR-0013 and FAILURE-MODES.md; consult counsel for applicability to your
> control environment.
"""
...
_KEY_ENV_VAR = "CRE_AUDIT_MI_PROXY_KEY"
_ZERO_KEY = b"\x00" * MIN_KEY_BYTES


class IntegrityVerificationError(RuntimeError):
    """Raised by ``AuditLedger.verify_chain`` when MI Proxy attestation fails.
...
class LocalMIProxy:
    ...
    def from_env(
        ...
        *,
        ...
        """Construct from the ``CRE_AUDIT_MI_PROXY_KEY`` env var.
    ...
    def verify_attestation(self, attestation: Attestation) -> bool:
        ...
        if attested_at.tzinfo is None:
            return False
        age = (datetime.now(timezone.utc) - attested_at).total_seconds()
