...
def sample_yaml_file(tmp_path: Path) -> Path:
    path = tmp_path / "compliance_rules.yaml"
...
class TestPatternLookup:
    ...
    def test_regulations_for_unknown_pattern_returns_empty(
        self, sample_yaml_file: Path
    ) -> None:
        loader = RegulationLoader.from_file(sample_yaml_file)
...
class TestReverseLookup:
    ...
    def test_patterns_satisfying_returns_empty_for_unknown(
        self, sample_yaml_file: Path
    ) -> None:
        loader = RegulationLoader.from_file(sample_yaml_file)
...
class TestCitationShape:
    def test_citation_carries_jurisdiction_and_effective_date(
        self, sample_yaml_file: Path
    ) -> None:
        loader = RegulationLoader.from_file(sample_yaml_file)
        regs = loader.regulations_for("fair_housing_preflight_gate")
        ...
        assert co_act.effective_date == "2027-01-01"
        assert co_act.statute == "SB 189"
...
class TestNistAndTreasuryMapping:
    ...
    YAML_WITH_MAPPINGS = textwrap.dedent(
        """
        version: "1.1.0"
        patterns:
          defcon_state_machine:
    ...
    def mapped_yaml_file(self, tmp_path: Path) -> Path:
        path = tmp_path / "compliance_rules.yaml"
        path.write_text(self.YAML_WITH_MAPPINGS, encoding="utf-8")
        return path
    ...
    def test_treasury_controls_for_returns_distinct_set(
    ...
    def test_nist_functions_for_unknown_pattern_returns_empty(
        self, mapped_yaml_file: Path
    ) -> None:
        loader = RegulationLoader.from_file(mapped_yaml_file)
