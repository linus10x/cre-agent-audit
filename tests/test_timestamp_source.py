...
def test_rfc3161_source_raises_when_fallback_disabled() -> None:
    ...
    # http.client error — all derived from OSError on CPython 3.10+.
    with pytest.raises(OSError):  # noqa: PT011
        src.stamp(b"x" * 32)


def test_rfc3161_source_rejects_unsupported_scheme() -> None:
    src = RFC3161TimestampSource(tsa_url="ftp://example/tsr", fallback_to_local_on_failure=False)
